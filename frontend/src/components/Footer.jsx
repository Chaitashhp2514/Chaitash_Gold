...
const Footer = () => {
  ...
  return (
    <footer className="relative border-t border-white/5 bg-[#09090a]">
      <div className="max-w-7xl mx-auto px-6 md:px-10 py-12">
        <div className="grid md:grid-cols-3 gap-10">
            ...
            <ul className="mt-4 grid grid-cols-2 gap-2">
              {navLinks.map((l) => (
                <li key={l.id}>
                  <a
                    href={`#${l.id}`}
                    className="text-zinc-300 text-sm hover:text-amber-300"
                  >
                    {l.label}
                  </a>
                </li>
          ...
          <div>
            ...
            <a
              ...
              className="mt-4 block text-zinc-100 hover:text-amber-300"
            >
              {profile.email}
            </a>
        ...
        <div className="mt-10 pt-6 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="mono text-xs text-zinc-500">
            ...
            <Heart className="inline w-3 h-3 mx-1 text-amber-400" />
            and a lot of coffee.
          </div>
          <button
            onClick={scrollTop}
            className="inline-flex items-center gap-2 text-sm text-zinc-400 hover:text-amber-300 transition-colors"
          >
            Back to top
